...
let landingAnimationStarted = false;
...
function startLandingCanvas() {
  if (landingAnimationStarted) return;
  ...
  function frame() {
    ...
    drawLandingManifold(ctx, rect.width, rect.height, tick);
...
function drawLandingManifold(ctx, width, height, tick) {
  const cx = width * 0.68;
  const cy = height * 0.36;
  ctx.save();
  ctx.translate(cx, cy);
  for (let ring = 0; ring < 6; ring += 1) {
    const rx = Math.max(120, width * 0.32 - ring * 42);
    const ry = Math.max(56, height * 0.18 - ring * 20);
    ctx.beginPath();
    ctx.ellipse(0, 0, rx, ry, Math.sin(tick + ring) * 0.08, 0, Math.PI * 2);
    ctx.strokeStyle = ring % 2 === 0 ? "rgba(45, 127, 249, 0.22)" : "rgba(52, 199, 123, 0.19)";
    ctx.lineWidth = 1.3;
    ctx.stroke();
  }

  for (let i = 0; i < 110; i += 1) {
    const angle = i * 0.39 + tick * (0.35 + (i % 5) * 0.025);
    const radius = 38 + (i % 19) * 17;
    const x = Math.cos(angle) * radius * 1.8;
    const y = Math.sin(angle * 1.31) * radius * 0.58;
    const isAuthority = i % 4 === 0;
    ctx.fillStyle = isAuthority ? "rgba(52, 199, 123, 0.72)" : "rgba(141, 120, 255, 0.56)";
    ctx.fillRect(x, y, isAuthority ? 4 : 3, isAuthority ? 4 : 3);
  }
  ctx.restore();

  const labels = ["archetype", "heuristics", "pre-label", "review", "retrieval"];
  const startX = Math.max(22, width * 0.08);
  const yBase = height * 0.82;
  const gap = Math.min(174, (width - startX * 2) / labels.length);
  labels.forEach((label, index) => {
    const x = startX + index * gap;
    const y = yBase + Math.sin(tick * 3 + index) * 7;
    const active = (Math.floor(tick * 24) + index) % 5 < 2;
    ctx.strokeStyle = active ? "rgba(52, 199, 123, 0.62)" : "rgba(151, 178, 218, 0.22)";
    ctx.fillStyle = active ? "rgba(52, 199, 123, 0.92)" : "rgba(151, 178, 218, 0.46)";
    ctx.lineWidth = 1.2;
    ctx.strokeRect(x, y, 112, 30);
    ctx.font = "700 11px system-ui";
    ctx.fillText(label.toUpperCase(), x + 10, y + 19);
    if (index < labels.length - 1) {
      ctx.beginPath();
      ctx.moveTo(x + 116, y + 15);
      ctx.lineTo(x + gap - 8, yBase + Math.sin(tick * 3 + index + 1) * 7 + 15);
      ctx.stroke();
    }
  });
}
